...
class ChatViewController: MessagesViewController{
    ...
    var isTyping = false
    let user = Sender.init(id: "1234", displayName: "owen")
    ...
    override func viewDidLoad() {
        ...
        ref?.child("Senduser").childByAutoId().setValue(user.id)
        ref?.child("displayname").childByAutoId().setValue(user.displayName)
...
extension ChatViewController: MessagesDataSource {
    ...
    func currentSender() -> Sender {
        return Sender(id: "1234", displayName: "Steven")
    }
...
extension ChatViewController: MessageInputBarDelegate {
    ...
    func messageInputBar(_ inputBar: MessageInputBar, didPressSendButtonWith text: String) {
        ...
        // Each NSTextAttachment that contains an image will count as one empty character in the text: String
         ...
         for component in inputBar.inputTextView.components {
           ...
           /* if let image = component as? UIImage {
                ...
                let imageMessage = Messagestruct(image: image, sender: currentSender(), messageId: UUID().uuidString, date: Date())
          ...
          if let text = component as? String {
            ...
            dateformatter.dateFormat = "MM/dd/yy h:mm a Z"
                ...
                let message = Messagestruct(attributedText: attributedText, sender: currentSender(), messageId: UUID().uuidString, date: Date())
                messageList.append(message)
            print(message.sentDate)
                ref?.child("chat").child("messageid").childByAutoId().setValue(message.messageId)
             ref?.child("chat").child("time").childByAutoId().setValue(dateformatter.string(from: message.sentDate))
                messagesCollectionView.insertSections([messageList.count - 1])
            }
        ...
        let sendmessage: String = text
 
        
        messagesCollectionView.scrollToBottom()
        
        ref?.child("chat").child("messages").childByAutoId().setValue(sendmessage)
